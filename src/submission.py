...
class MultiTaskNet(nn.Module):
    ...
    def init_shared_user_and_item_embeddings(self, num_users, num_items, embedding_dim):
        ...
        U = Q = None
        ### START CODE HERE ###
        # Embeddings for users and items with normal distribution
    ...
    def init_user_and_item_bias(self, num_users, num_items):
        ...
        A = B = None
        ### START CODE HERE ###
        # Embeddings for user biases
        A = ZeroEmbedding(num_users, 1)
        # Embeddings for item biases
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        ### START CODE HERE ###
        layers = []
        # Input size - 96 because of concatenation of [u_i, q_j, u_i * q_j]
        input_size = 3 * self.embedding_dim
        # Layer sizes starting from the second element to create Linear layers followed by ReLU activations
        for size in layer_sizes[1:]:
            layers.append(nn.Linear(input_size, size))
            layers.append(nn.ReLU())
            input_size = size
        # Final Linear layer with output size of 1
        layers.append(nn.Linear(input_size, 1))
        # Sequential model from the layers
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        user_embeds = self.U(user_ids)
        item_embeds = self.Q(item_ids)
        # User and item biases without extra dimensions
        user_bias = self.A(user_ids).squeeze()
        item_bias = self.B(item_ids).squeeze()
        # The dot product of user and item embeddings with biases
        predictions = torch.sum(user_embeds * item_embeds, dim=1) + user_bias + item_bias
        # Concat embeddings and their element-wise product for MLP input
        mlp_input = torch.cat([user_embeds, item_embeds, user_embeds * item_embeds], dim=1)
        # Get the score without extra dimensions
        score = self.mlp_layers(mlp_input).squeeze()
        ### END CODE HERE ###
        return predictions, score
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        # User and item embeddings for factorization
        user_embeds_fact = self.U_fact(user_ids)
        item_embeds_fact = self.Q_fact(item_ids)
        # User and item biases without extra dimensions
        user_bias = self.A(user_ids).squeeze()
        item_bias = self.B(item_ids).squeeze()
        # The dot product of user and item embeddings with biases
        predictions = torch.sum(user_embeds_fact * item_embeds_fact, dim=1) + user_bias + item_bias
        # User and item embeddings for regression
        user_embeds_reg = self.U_reg(user_ids)
        item_embeds_reg = self.Q_reg(item_ids)
        #Concat embeddings and element-wise product
        mlp_input = torch.cat([user_embeds_reg, item_embeds_reg, user_embeds_reg * item_embeds_reg], dim=1)
        # Get the score without extra dimensions
        score = self.mlp_layers(mlp_input).squeeze()
        ### END CODE HERE ###
        return predictions, score
